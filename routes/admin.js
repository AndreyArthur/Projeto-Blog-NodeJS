...
router.post('/categorias/new', eAdmin, (req, res) => {
  ...
  // Validação
    ...
    if(req.body.slug.length < 2 || req.body.nome.length < 2) {
      erros.push({texto: 'Nome ou Slug muito curto'})
    }
  ...
  Categoria.findOne({_id: req.body.id})
    .then( (categoria) => {
      ...
      if(erros.length > 0) {
        ...
        categoria.save()
          .then( () => {
            req.flash('succes_msg', eAdmin, 'Categoria editada com sucesso')
            res.redirect('/admin/categorias')
          }).catch( (err) => {
      ...
      req.flash('error_msg', eAdmin, 'Houve um erro ao editar a categoria')
      res.redirect('/admin/categorias')
    })
})
